...
//        key := pathJoin(uploadIDDir, fs.encodePartFile(partID, etag, data.ActualSize()))
	key :=  bucket + "/" + object + "/" +  strconv.Itoa(partID) +"." + object
	...
	// metadata file
        ...
        metakey := "meta." + strconv.Itoa(partID)+ "." +  key
...
//        startKey := "meta." + uploadIDDir + "/" //"meta.bucket."
	startKey := "meta." + bucket + "/" + object
	endKey := startKey +"0" 
//        endKey :=   "meta." + uploadIDDir +"0"
	println(" START/END KEY", startKey, endKey)
        ...
        for _, key := range keys {
                log.Println("ENTRY: ", string(key))
                if string(key) == ("meta." + bucket + "/" + object + "/" +  fs.metaJSONFile) {
			log.Println("***** CONTINUE******")
                        continue
                }
                partNumber, etag1, _, derr := fs.decodePartFile(string(key))
                if derr != nil {
                        // Skip part files whose name don't match expected format. These could be backend filesystem specific files.
                        continue
                }
