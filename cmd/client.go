...
func (c *Client) CGetMeta(key string, acmd Opts) (*C.char, *C.char, uint32, error) {
	metaKey := "meta." + key
...
func (c *Client) CGet(key string, acmd Opts) (*C.char, *C.char, uint32, error) {
	mutexCmd.Lock()
	...
	var status int
	var cvalue *C.char
        cvalue = C.Get(1, cKey, &psv, &ptr, (*C.int)(unsafe.Pointer(&size)), (*C.int)(unsafe.Pointer(&status)))
	mutexCmd.Unlock()
	var err error = nil
