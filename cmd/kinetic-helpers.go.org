...
func koRemoveFile(ctx context.Context, filePath string) (err error) {
	if filePath == "" {
...
func koRemoveAll(ctx context.Context, dirPath string) (err error) {
	if dirPath == "" {
		logger.LogIf(ctx, errInvalidArgument)
		return errInvalidArgument
	}
...
func koRemoveDir(ctx context.Context, dirPath string) (err error) {
	if dirPath == "" {
		logger.LogIf(ctx, errInvalidArgument)
		return errInvalidArgument
	}
...
func koMkdir(ctx context.Context, dirPath string) (err error) {
	if dirPath == "" {
		logger.LogIf(ctx, errInvalidArgument)
		return errInvalidArgument
	}
...
// koStat is a low level call which validates input arguments
...
// koStatFileDir, koStatFile, koStatDir.
func koStat(ctx context.Context, statLoc string) (os.FileInfo, error) {
	if statLoc == "" {
		logger.LogIf(ctx, errInvalidArgument)
...
func koStatVolume(ctx context.Context, volume string) (os.FileInfo, error) {
	fmt.Println(" koStatVolume: ", volume)
	fi, err := koStat(ctx, volume)
	if err != nil {
		if os.IsNotExist(err) {
...
// into a more KO object layer friendly form, converts
// known errors into their typed form for top level
// interpretation.
func osErrToKOFileErr(err error) error {
	if err == nil {
		return nil
	}
	if os.IsNotExist(err) {
...
func koStatDir(ctx context.Context, statDir string) (os.FileInfo, error) {
	fi, err := koStat(ctx, statDir)
	if err != nil {
		err = osErrToKOFileErr(err)
		if err != errFileNotFound {
			logger.LogIf(ctx, err)
...
func koStatFile(ctx context.Context, statFile string) (os.FileInfo, error) {
	fi, err := koStat(ctx, statFile)
	if err != nil {
		err = osErrToKOFileErr(err)
		if err != errFileNotFound {
			logger.LogIf(ctx, err)
...
func koIsFile(ctx context.Context, filePath string) bool {
	fi, err := koStat(ctx, filePath)
	if err != nil {
		return false
	}
...
func koOpenFile(ctx context.Context, readPath string, offset int64) (io.ReadCloser, int64, error) {
	if readPath == "" || offset < 0 {
		logger.LogIf(ctx, errInvalidArgument)
		return nil, 0, errInvalidArgument
	}
	fmt.Println("  koOpenFile ", readPath)
	...
	if err != nil {
		return nil, 0, osErrToKOFileErr(err)
	}
	...
	sizeRead, err := ioutil.ReadFile(readPath)
	size := int64(binary.LittleEndian.Uint64(sizeRead))
	return fr, size, nil
}
...
func koCreate(ctx context.Context, obj string, reader io.Reader, buf []byte, fallocSize int64) (int64, error) {
	log.Trace("koCreate ", obj)
	if obj == "" || reader == nil {
		logger.LogIf(ctx, errInvalidArgument)
	...
	writer, err := lock.Open(obj, os.O_CREATE|os.O_WRONLY, 0666)
	if err != nil {
		return 0, osErrToKOFileErr(err)
	}
	defer writer.Close()

	// Fallocate only if the size is final object is known.
	//if fallocSize > 0 {
	//	if err = koFAllocate(int(writer.Fd()), 0, fallocSize); err != nil {
	//		logger.LogIf(ctx, err)
	//		return 0, err
	//	}
	//}

	var size uint64 = uint64(fallocSize)
	buffer := make([]byte, 8)
	binary.LittleEndian.PutUint64(buffer, size)
	//log.Trace("BUFFER SIZE ", buffer)
	...
	if buf != nil {
		...
		writer.Write(buffer)
		bytesWritten, err = readToBuffer(reader, buf)
		if err != nil {
...
// koFAllocate is similar to Fallocate but provides a convenient
// wrapper to handle various operating system specific errors.
func koFAllocate(fd int, offset int64, len int64) (err error) {
...
func koSimpleRenameFile(ctx context.Context, sourcePath, destPath string) error {
...
func koRenameFile(ctx context.Context, sourcePath, destPath string) error {
	if err := checkPathLength(sourcePath); err != nil {
		logger.LogIf(ctx, err)
		return err
	}
...
// koDeleteFile is a wrapper for deleteFile(), after checking the path length.
func koDeleteFile(ctx context.Context, basePath, deletePath string) error {
...
// koRemoveMeta safely removes a locked file and takes care of Windows special case
func koRemoveMeta(ctx context.Context, basePath, deletePath, tmpDir string) error {
	...
	if runtime.GOOS == globalWindowsOSName {
		...
		koRenameFile(ctx, deletePath, tmpPath)

		// Proceed to deleting the directory if empty
		koDeleteFile(ctx, basePath, pathutil.Dir(deletePath))

		// Finally delete the renamed file.
		return koDeleteFile(ctx, tmpDir, tmpPath)
	}
	return koDeleteFile(ctx, basePath, deletePath)
}
