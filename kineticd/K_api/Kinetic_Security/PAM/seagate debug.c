...
PAM_EXTERN int pam_sm_authenticate( pam_handle_t *pamh, int flags,int argc, const char **argv ) {
        ...
        char 	Challenge_Response[100];  /* make larger enough to hold at least 32 bytes */
        ...
        retval = ks_mandiags(Device_Id, KS_PAM_CHALLENGE, lock_request, Challenge_Response, Session_Info);
          ...
          resp = NULL ;
int tretval;
          if( (tretval = converse(pamh, 1 , pmsg, &resp))!=PAM_SUCCESS ) {
            return tretval ;
          }
        ...
        /* Just look for AUTHORIZAION for this version of code CA 6/24/14 */
        lock_request = AUTHORIZE_PORT;

          sprintf(Temp_Str, "CR='%s'\n",Challenge_Response);
          /* setting up conversation call prompting for Lock direction */
          pmsg[0] = &msg[0] ;
          msg[0].msg_style = PAM_TEXT_INFO ;  /* Display a message */
          msg[0].msg = Temp_Str;
