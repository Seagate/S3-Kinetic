...
//----------------------------------------------------------
//
// Header: Math/sc_aeskey.c
// Date: 2010/01/07
// Author: sumanth.j.venkata
//
// This is trust math module AES key code.
//
//----------------------------------------------------------
...
#if (BLOCK_SIZE == 16) && (ENC_UNROLL != NONE_)
...
{   aes_32t    ss[8]; 
...
#if (BLOCK_SIZE == 16) && (DEC_UNROLL != NONE_)
    ...
    case 16:    kdf4(cx->k_sch, 0); kd4(cx->k_sch, 1); 
                ...
                cx->n_rnd = 10; break;   // lint does not allow Macro ff to contain another macro word_in 
    case 24:    cx->k_sch[4] = ff(ss[4] = word_in(in_key + 16));   //lint !e666    
                cx->k_sch[5] = ff(ss[5] = word_in(in_key + 20));   //lint !e666      
                kdf6(cx->k_sch, 0); kd6(cx->k_sch, 1); 
                kd6(cx->k_sch, 2); kd6(cx->k_sch, 3);
                kd6(cx->k_sch, 4); kd6(cx->k_sch, 5); 
                kd6(cx->k_sch, 6); kdl6(cx->k_sch, 7); 
                cx->n_rnd = 12; break;
    case 32:    cx->k_sch[4] = ff(ss[4] = word_in(in_key + 16));   //lint !e666         
                cx->k_sch[5] = ff(ss[5] = word_in(in_key + 20));   //lint !e666         
                cx->k_sch[6] = ff(ss[6] = word_in(in_key + 24));   //lint !e666         
                cx->k_sch[7] = ff(ss[7] = word_in(in_key + 28));   //lint !e666         
