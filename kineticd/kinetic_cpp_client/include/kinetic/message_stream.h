...
class MessageStreamInterface {
    ...
    typedef enum {
        ...
        MessageStreamReadStatus_NO_SPACE
    } MessageStreamReadStatus;

    virtual ~MessageStreamInterface() {}
    virtual MessageStreamReadStatus ReadHeader(uint32_t *message_size, uint32_t *value_size, int* err) = 0;
    virtual MessageStreamReadStatus ReadMessageAPI(::google::protobuf::Message *message, uint32_t msgSize) = 0;
    virtual MessageStreamReadStatus ReadValueToBuffer(char* valBuff, uint32_t valSize) = 0;
    virtual MessageStreamReadStatus ReadValue(IncomingValueInterface** value, uint32_t valSize) = 0;
...
class MessageStream : public MessageStreamInterface {
    ...
    MessageStreamReadStatus ReadHeader(uint32_t *message_size, uint32_t *value_size, int* err);
    MessageStreamReadStatus ReadMessageAPI(::google::protobuf::Message *message, uint32_t msgSize);
    MessageStreamReadStatus ReadValueToBuffer(char* valBuff, uint32_t valSize);
    MessageStreamReadStatus ReadValue(IncomingValueInterface** value, uint32_t valSize);
