...
HandlerKey NonblockingKineticConnection::SetClusterVersion(int64_t new_cluster_version,
    ...
    request->mutable_body()->mutable_setup()->set_setupoptype(Command_Setup_SetupOpType_CLUSTER_VERSION_SETUPOP);
...
HandlerKey NonblockingKineticConnection::UpdateFirmware(
    ...
    msg->set_authtype(Message_AuthType_HMACAUTH);
    unique_ptr<Command> request = NewCommand(Command_MessageType_SETUP);
    request->mutable_body()->mutable_setup()->set_setupoptype(Command_Setup_SetupOpType_FIRMWARE_SETUPOP);
...
HandlerKey NonblockingKineticConnection::SetACLs(const shared_ptr<const list<ACL>> acls,
    ...
    request->mutable_body()->mutable_security()->set_securityoptype(Command_Security_SecurityOpType_ACL_SECURITYOP);
        ...
        const shared_ptr<SimpleCallbackInterface> callback)
{
    ...
    unique_ptr<Command> request = NewCommand(Command_MessageType_SECURITY);
    request->mutable_body()->mutable_security()->set_securityoptype(Command_Security_SecurityOpType_LOCK_PIN_SECURITYOP);
    ...
    const shared_ptr<SimpleCallbackInterface> callback)
{
    ...
    unique_ptr<Command> request = NewCommand(Command_MessageType_SECURITY);
    request->mutable_body()->mutable_security()->set_securityoptype(Command_Security_SecurityOpType_ERASE_PIN_SECURITYOP);
