...
MessageStreamInterface::MessageStreamReadStatus MessageStream::ReadValueToBuffer(char* valBuff, uint32_t valSize) {
    int err = 0;
    if (byte_stream_->Read(valBuff, valSize, &err)) {
        return MessageStreamReadStatus_SUCCESS;
    }
    return MessageStreamReadStatus_INTERNAL_ERROR;
}
MessageStreamInterface::MessageStreamReadStatus MessageStream::ReadValue(IncomingValueInterface** value, uint32_t valSize) {
    *value = byte_stream_->ReadValue(valSize);
    if ((*value != NULL) && ((*value)->GetUserValue() == NULL)) {
        return MessageStreamReadStatus_NO_SPACE;
    }
    return MessageStreamReadStatus_SUCCESS;
}

MessageStreamInterface::MessageStreamReadStatus MessageStream::ReadMessageAPI(::google::protobuf::Message *message, uint32_t msgSize) {
    if (msgSize > max_message_size_bytes_) {
        return MessageStreamReadStatus_TOO_LARGE;
    }

    // Now the message
    char *message_bytes = new char[msgSize];
    int err = 0;
    if (!byte_stream_->Read(message_bytes, msgSize, &err)) {
        LOG(WARNING) << "Unable to read message";
        delete[] message_bytes;
        return MessageStreamReadStatus_INTERNAL_ERROR;
    }

    if (!message->ParseFromArray(message_bytes, msgSize)) {
...
MessageStreamInterface::MessageStreamReadStatus MessageStream::ReadMessage(
        ::google::protobuf::Message *message,
        IncomingValueInterface** value) {
    // First the header
    uint32_t message_size, value_size;
    int err = 0;
    if (!ReadHeader(&message_size, &value_size, &err)) {
        return MessageStreamReadStatus_INTERNAL_ERROR;
    }
    ...
    if (!byte_stream_->Read(message_bytes, message_size, &err)) {
        LOG(WARNING) << "Unable to read message";
        delete[] message_bytes;
        return MessageStreamReadStatus_INTERNAL_ERROR;
    }
    ...
    MessageStreamReadStatus status = MessageStreamReadStatus_SUCCESS;
    *value = byte_stream_->ReadValue(value_size);
    if (*value != NULL && (*value)->GetUserValue() == NULL) {
        status = MessageStreamReadStatus_NO_SPACE;
//        status = MessageStreamReadStatus_INTERNAL_ERROR;
        /*
        if (smr::Disk::_status == smr::Disk::DiskStatus::NO_SPACE) {
            status = MessageStreamReadStatus_NO_SPACE;
        } else {
            status = MessageStreamReadStatus_INTERNAL_ERROR;
        }
...
bool MessageStreamFactory::NewMessageStream(int fd, bool use_ssl, SSL *ssl, uint32_t max_message_size_bytes,
    ...
    if (use_ssl) {
        ...
        VLOG(1) << "Successfully performed SSL handshake";
        *message_stream = new MessageStream(max_message_size_bytes, new SslByteStream(ssl, value_factory_));
    } else {
        *message_stream =
...
MessageStreamInterface::MessageStreamReadStatus MessageStream::ReadHeader(uint32_t *message_size, uint32_t *value_size, int* err) {
    char header[9];
    if (!byte_stream_->Read(header, sizeof(header), err)) {
        return MessageStreamReadStatus_INTERNAL_ERROR;
    }
