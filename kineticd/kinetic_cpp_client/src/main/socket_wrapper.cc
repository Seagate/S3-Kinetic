...
SocketWrapper::SocketWrapper(const std::string& host, uint port, bool use_ssl, bool nonblocking)
...
SocketWrapper::~SocketWrapper() {
    if (fd_ == -1) {
        VLOG(1) << "Not connected so no cleanup needed";
    } else {
        VLOG(1) << "Closing socket with fd " << fd_;
        if (close(fd_)) {
