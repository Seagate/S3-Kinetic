...
static double MaxBytesForLevel(int level) {
  ...
  double result = 4 * kTargetFileSize;  // Result for both level-0 and level-1
  while (level > 1) {
    result *= 10;
    level--;
  }
...
Status Version::Get(const ReadOptions& options,
                    const LookupKey& k,
                    std::string* value,
                    GetStats* stats, bool using_bloom_filter) {
...
void Version::GetOverlappingInputs(
  ...
  if (level == 0) {
    ...
    for (size_t i = index; i < files_[level].size(); i++){
      FileMetaData* f = files_[level][i];
      const Slice file_start = f->smallest.user_key();
      if (end != NULL && user_cmp->Compare(file_start, user_end) > 0){
          return;
      }
      inputs->push_back(f);
    }
...
void VersionSet::Finalize(Version* v) {
  ...
  v->manifest_compaction_score_ = current_->manifest_compaction_score_;
}
...
Status VersionSet::CompactManifest() {
    ...
    WritableFile* newDescriptorFile = NULL;
    log::Writer* newDescriptorLog = NULL;
    Status status = env_->NewWritableFile(newManifestFileName, &newDescriptorFile);
    if (status.ok()) {
       newDescriptorLog = new log::Writer(newDescriptorFile);
...
Compaction* VersionSet::PickCompaction() {
  ...
  if (size_compaction) {
    ...
    if (c->inputs_[0].empty()) {
      ...
      Log(options_->info_log, "==== Compacting MANIFEST...\n");
      CompactManifest();
      Log(options_->info_log, "==== Complete Compacting MANIFEST\n");
      return NULL;
...
bool Compaction::IsBaseLevelForKey(const Slice& user_key) {
  // Counter variable for when there no files in the specified level.
  int no_file_count = 0;
  // Maybe use binary search to find right entry instead of linear search?
  const Comparator* user_cmp = input_version_->vset_->icmp_.user_comparator();
  for (int lvl = level_ + 2; lvl < config::kNumLevels; lvl++) {
    const std::vector<FileMetaData*>& files = input_version_->files_[lvl];
    // Increment counter if no files.
    if (files.size() == 0 ) { no_file_count++;}
  ...
  // If the count is 5, that means there were no files between level 2 and 6.
  // Meaning the stale entry was removed at level 1.
  if (no_file_count == 5) { LevelLogEvent(options_->info_log, 1);}
  return true;
}
