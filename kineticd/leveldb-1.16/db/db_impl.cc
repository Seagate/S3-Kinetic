...
void DBImpl::BackgroundCompaction() {
  ...
  if (c == NULL) {
    ...
    LogCompactionEvent(options_.info_log);

    int keys_before = KeyCount("Before");
    status = DoCompactionWork(compact);
    int keys_after = KeyCount("After");
    assert(keys_before == keys_after && "Keys do not match before and after compaction.");
...
Status DBImpl::Get(const ReadOptions& options,
                   const Slice& key,
                   std::string* value, bool using_bloom_filter) {
  Status s;
  MutexLock l(&mutex_);
  ...
  while (true) {
    if (!bg_error_.ok()) {
      ...
      break;
        ...
        allow_delay &&
        versions_->NumLevelFiles(0) >= config::kL0_CompactionTrigger) {
      // We are getting close to hitting a hard limit on the number of
      // L0 files.  Rather than delaying a single write by several
      // seconds when we hit the hard limit, start delaying each
      // individual write by 2^(number of file at level 0 minus kL0_CompactionTrigger) ms
      // to reduce latency variance.  Also,
      // this delay hands over some CPU to the compaction thread in
      // case it is sharing the same core as the writer.
      int delayTime = (1 << (versions_->NumLevelFiles(0) - config::kL0_CompactionTrigger));
      mutex_.Unlock();
      delayTime *= 1000;
...
int DBImpl::KeyCount(const std::string event) {
  #ifndef KEY_COUNT
  return 0;
  #endif
  mutex_.Unlock();
  Iterator* iter = NewIterator(leveldb::ReadOptions());
  ...
  delete iter;
  Log(options_.info_log, "Number of keys present %s: %d\n", event.c_str(), key_count);
