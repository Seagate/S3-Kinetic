...
using com::seagate::kinetic::client::proto::Command_Setup_SetupOpType_FIRMWARE_SETUPOP;
using com::seagate::kinetic::client::proto::Command_Setup_SetupOpType_CLUSTER_VERSION_SETUPOP;
...
using com::seagate::kinetic::client::proto::Command_Security_SecurityOpType_ACL_SECURITYOP;
using com::seagate::kinetic::client::proto::Command_Security_SecurityOpType_ERASE_PIN_SECURITYOP;
using com::seagate::kinetic::client::proto::Command_Security_SecurityOpType_LOCK_PIN_SECURITYOP;
...
TEST_F(NonblockingKineticConnectionTest, GetKeyRangeWorks) {
    ...
    ASSERT_EQ(1234U, message.body().range().maxreturned());
}
...
TEST_F(NonblockingKineticConnectionTest, SetClusterVersionSendsCorrectVersion) {
    ...
    EXPECT_EQ(Command_Setup_SetupOpType_CLUSTER_VERSION_SETUPOP, message.body().setup().setupoptype());
...
TEST_F(NonblockingKineticConnectionTest, GetLogParsesMessageCorrectly) {
    ...
    EXPECT_EQ(123U, drive_log.capacity.nominal_capacity_in_bytes);
    ...
    EXPECT_EQ(111U, drive_log.operation_statistics.front().count);
    EXPECT_EQ(1024U, drive_log.operation_statistics.front().bytes);
...
TEST_F(NonblockingKineticConnectionTest, FirmwareUpdateSendsFirmwareContents) {
    ...
    EXPECT_EQ(Command_MessageType_SETUP, message.header().messagetype());
    EXPECT_EQ(Command_Setup_SetupOpType_FIRMWARE_SETUPOP, message.body().setup().setupoptype());
}
...
TEST_F(NonblockingKineticConnectionTest, SetACLsBuildsCorrectMessage) {
    ...
    EXPECT_EQ(Command_Security_SecurityOpType_ACL_SECURITYOP, message.body().security().securityoptype());
    ...
    EXPECT_EQ(0U, proto_acl1_scope1.offset());
    ...
    EXPECT_EQ(100U, proto_acl1_scope2.offset());
    ...
    EXPECT_EQ(0U, proto_acl2_scope1.offset());
...
TEST_F(NonblockingKineticConnectionTest, SetErasePinBuildsCorrectMessageForNoCurrentPin) {
    ...
    EXPECT_EQ(Command_MessageType_SECURITY, message.header().messagetype());
    EXPECT_EQ(Command_Security_SecurityOpType_ERASE_PIN_SECURITYOP, message.body().security().securityoptype());
...
TEST_F(NonblockingKineticConnectionTest, P2PPushBuildsCorrectMessage) {
    ...
    EXPECT_EQ(1234U, message.body().p2poperation().peer().port());
    ...
    EXPECT_EQ(1235U, operation0.p2pop().peer().port());
    ...
    EXPECT_EQ(1236U, operation0_nested.p2pop().peer().port());
